<template>
  <div>
    ...
    <b-button v-if="auth_permission.user_create" @click="add_user" style="margin-bottom:10px;">Create New Author </b-button>
    <!-- Author List  -->
    <div class="card">
         <table class="table table-sm">
             ...
             <tbody v-if="auth_permission.user_view || auth_permission.user_viewall">
                 <tr v-for="(item,key) in user_list.filter(v => v.role.name == 'author')" :key="key">
                     <td>{{key + 1}}</td>
                     <td>{{item.name}}</td>
