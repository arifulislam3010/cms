<template>
    <div class="container">
        ...
        <span class="upload_content" v-if="upload_process" @click="cancelUpload">Upload Cancel</span>
    </div>
</template>
<script>
    ...
    const myUploadProgress = (myFileId) => (progress) => {
        let percentage = parseInt( Math.floor( (progress.loaded * 100) / progress.total ) );
    ...
    export default {
        ...
        methods:{
            ...
            cancelUpload(){
                swal({
                    title: "Are you sure to cancel upload?",
                    // text: "Once updated, you will not be able to recover this imaginary data!",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                    })
                    .then((willDelete) => {
                    if (willDelete) {
                        $('.content-upload-system-block .action-btn .content-submit-btn').css('display','none');
            ...
            uploadFile(){
                 ...
                 */
                axios.post( '/'+ this.username +'/content-bank/api/upload',
                    formData,
                    {
                        headers: {
                        ...
                        onUploadProgress: myUploadProgress(upload_index).bind(this)
                    }
                ).then((response) => {            
