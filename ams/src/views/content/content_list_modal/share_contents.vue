...
<script>
    ...
    export default {
        props: ['username','storage_path','content_image_path'],
        ...
        methods:{            
            loadData(req_data_type, page = 1, keyword = this.keyword){
                 ...
                 */
                axios.get( `/` + this.username + `/content-bank/api/load_data/` + req_data_type + `?` + (keyword!=''?`keyword=` + keyword + `&`:``) + `page=` + page )
                .then((response) => {                    
