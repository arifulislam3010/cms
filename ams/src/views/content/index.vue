        ...
        <form @submit.prevent="addArea" >
            <b-row>
              <div class="col-md-12 content_type">
                  <span :class="{ content_active : content_type == 'image' }" @click="ContentTypeChange('image')" data-toggle="tooltip" data-placement="top" title="Image">
                    <i class="fa fa-image"></i>
                  </span>
                  <span  :class="{ content_active : content_type == 'video' }" @click="ContentTypeChange('video')" data-toggle="tooltip" data-placement="top" title="Video">
                    <i class="fa fa-video-camera" aria-hidden="true"></i>
                  </span>
                  <span :class="{ content_active : content_type == 'audio' }" @click="ContentTypeChange('audio')" data-toggle="tooltip" data-placement="top" title="Audio">
                    <i class="fa fa-volume-up"></i>
                  </span>
                  <span :class="{ content_active : content_type == 'file' }" @click="ContentTypeChange('file')" data-toggle="tooltip" data-placement="top" title="File">
              ...
              <div class="col-md-12 row">
                  ...
                  <div class="col-md-3">
                      <div class="from-group row">
                            <div class="col-sm-12">
                                  <div v-if="list.file==''" class="video_upload logo_upload" @click="$refs.ContentFile.click()">
                                  <p>Choose file (file type> PDF/MP4/jpg/png/mp3, size > 100MB )</p>
                                    <i class="fa fa-cloud-upload-alt new_btn"></i>
                                  <input  type="file" ref="ContentFile" class="file-input" style="display: none" v-on:change="onContentChange">
                                  <small id="emailHelp" class="form-text text-muted" style="color:read" v-if="errors.file">{{errors.file[0]}}</small>
                                  </div>
                                  <div v-else class="video_upload logo_upload">
                                    ...
                                    <p style="color:red;" v-for="(servererror,k) in servererrors.file" v-bind:key="'u'+k">{{servererror}}</p>
                            </div>
                            <div class="col-sm-12">
                                <label>Title  : </label>
  ...
  components: {
    ...
    data(){
        return{
            ...
            video_ext:['mp4', 'mpeg', 'avi', 'mov', 'mpeg-4', 'wmv', 'mpeg-ps', 'flv', '3gpp', 'webm'],
    ...
    methods:{
      ...
      getData(){
        this.$store.dispatch('GET_CONTENT',this.search)
          .then(response=>{
      ...
      download_file(item){
             this.detail_url = this.base_url + '/uploads/PLOT_INFO_DOCUMENT/' + item.name
             axios.get(this.detail_url,{responseType: 'blob'})
            .then((response) => {
                return 
          ...
          del(key,id){
            ...
            this.$store.dispatch(DEL_PLOT_INFO_FILE,ob)
            .then(response=>{
                this.$iziToast.success({position:'topRight',title:'Ok',message:"deleted "})
            })
            .catch(error=>{
                console.log(error)
                this.$iziToast.error({position:'topRight',title:'Ok',message:"error "})
            })
          },
        ...
        upload(){
          console.log(this.list);
                ...
                .then(response => {
                     this.$iziToast.success({position:'topRight',title:'Ok',message:"file uploaded "})
                ...
                .catch((error) => {
                  this.responseLoading=false;
                  this.$iziToast.error({position:'topRight',title:'Ok',message:"error uploading file"})
